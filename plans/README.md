...
shape candidates beyond the bounded Spectrum Switch and Astryx core Button
adapters, but it does not create support or proof authority without
target-specific schemas, fixtures, diagnostics, commands, artifacts or reports,
evidence, CI gates, and passing evidence.
...
The Spectrum Checkbox catalog target is implemented for one separately locked
source byte from the pinned Spectrum package. It consumes intact P2 evidence,
preserves every accepted P2 component and token record, and emits a distinct
governed catalog with Checkbox plus one desktop token. Passing evidence remains
`review_required`. It does not change P2 or prove full Spectrum support, live
connectors, self-serve UI, runtime accessibility compliance, production
adapters, SurfaceOps expansion, JudgmentKit, or A2UI.

The source-independent design-system compiler target is implemented for two
separately locked, data-only adapters: Spectrum Switch and official Astryx core
Button. Both use one shared ingestion kernel and one shared inert portable
consumer. Passing evidence requires matching implementation hashes, no
source-specific implementation modules, complete mapping coverage, a
hash-bound catalog receipt, and allowed, blocked, and review-required consumer
outcomes. It does not change P2 or Checkbox, make legacy P3-P5 consumers
portable, or prove broad Spectrum or Astryx support, runtime accessibility
compliance, live connectors, self-serve UI, production adapters, SurfaceOps
...
source-family component-identity mapping, Spectrum Checkbox catalog, source-independent design-system compiler, designer-workflow trace,
...
2. The 18 accepted target evidence files declared by the capability fixture
