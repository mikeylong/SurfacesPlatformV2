...
19. [Spectrum Switch Catalog Proof Target](spectrum-switch-catalog.md)
...
The Spectrum Switch catalog target is implemented for one Switch-specific
locked source byte downstream of passing P2 and Checkbox evidence. It preserves
36 Checkbox-baseline records by JCS hash and adds only Switch plus one desktop
token to a new catalog identity. Passing evidence remains `review_required`.
It does not change P2 or Checkbox or prove runtime behavior, accessibility
compliance, full Spectrum support, live connectors, self-serve UI, production
adapters, SurfaceOps expansion, JudgmentKit, A2UI, or production readiness.
...
source-family component-identity mapping, Spectrum Checkbox catalog, Spectrum Switch catalog, designer-workflow trace,
