...
| P4 | Planned as deterministic review and judgment proof; not implemented until P4 schemas, fixtures, command, artifacts, demo, tests, `artifacts/p4/evidence.json`, and `check:p4:ci` exist and pass | Explain the planned SurfaceOps decision ledger, JudgmentKit-shaped evaluation report, review/judgment report, P3 evidence and review queue inputs, exact diagnostic fixture coverage, evidence authority, second-review semantics, planned `node bin/interfacectl.js` invocation, proof-bearing `check:p4:ci` once implemented, SurfaceOps ownership of approve/reject/request-changes/defer decision records, JudgmentKit evaluation-only findings, and no live SurfaceOps console, live JudgmentKit invocation, work-order execution, production adapter, A2UI, or authority override in P4 |
...
- P4 implementation start gate: pending. Record clean post-merge `main`, `npm run check:p0:ci`, `npm run check:p1:ci`, `npm run check:p2:ci`, `npm run check:p3:ci`, accepted P3 evidence hash, and proof-bearing gate logs before claiming P4 implementation work.
- P4 public/docs follow-up: explain the planned review and judgment proof without implying live SurfaceOps persistence, live JudgmentKit MCP or connector invocation, work-order execution, production adapters, P5/A2UI scope, or authority override.
