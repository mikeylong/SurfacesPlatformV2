...
| --- | --- | --- |
| Cross-product | Product portfolio boundaries and usability/value evidence tracked in proof docs | Explain platform authority, product portfolio boundaries, lifecycle enforcement, usability/value evidence cards, SurfaceOps versus `kanban.cards` positioning, JudgmentKit evaluation-only role, A2UI future-target boundary, and the rule that passing evidence, not demos or docs, proves implemented behavior |
...
- Product portfolio docs follow-up: expose [Product Portfolio Boundaries](product-portfolio-boundaries.md) and [Usability And Value Evidence Plan](usability-value-evidence.md) in the future `surfaces.dev` docs package. Keep `VISION.md` canonical, keep these docs subordinate, and keep demos presentation-only.
