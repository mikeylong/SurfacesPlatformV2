...
| --- | --- | --- |
| Cross-product | Product portfolio boundaries, roadmap proof snapshot, and usability/value evidence tracked in proof docs | Explain platform authority, product portfolio boundaries, lifecycle enforcement, current P0-P5 proof snapshot, usability evidence cards, SurfaceOps versus `kanban.cards` positioning, JudgmentKit evaluation-only role, A2UI future-target boundary, and the rule that passing evidence, not demos or docs, proves implemented behavior |
| P0 | Implemented as deterministic synthetic catalog proof when `artifacts/p0/evidence.json` passes | Explain catalog proof, fixture-only extraction, diagnostics, evidence, and no real design-system ingestion claim |
...
| P5 | Implemented as deterministic inert `surfaces-protocol-static` protocol-envelope proof when `artifacts/p5/protocol/evidence.json` passes | Explain `protocol-target-selection.v0`, `protocol-projection.v0`, `protocol-envelope.v0`, protocol adapter report, accepted P2 catalog/evidence inputs, accepted P4 evidence/decision-ledger/review-report inputs, accepted upstream boundary refresh procedure, exact valid/review/invalid/mutation fixture coverage, diagnostic registry rows, `node bin/interfacectl.js surfaces protocol proof` invocation, generated demo, proof-bearing `check:p5:protocol:ci`, evidence authority, review-required non-execution, stale-output and untracked guards, and no production adapter/API/SDK/live protocol/A2UI/live SurfaceOps/live JudgmentKit support in this P5 slice |
...
- P5 public/docs follow-up: explain the implemented `surfaces-protocol-static` proof without implying production adapters, protocol APIs, SDKs, live protocol services, A2UI export/conformance, live SurfaceOps, live JudgmentKit, action execution, or authority override. Include the accepted upstream boundary refresh procedure as explicit-review maintenance guidance, not an automatic latest-upstream behavior. Future P5 targets still require target-specific schemas, fixtures, diagnostics, command implementation, artifacts, target report, final evidence, demo, CI wiring, non-goals, acceptance criteria, and passing evidence before support can be claimed.
