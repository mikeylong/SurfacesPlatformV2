...
- SurfaceOps is the standalone review-product direction. `surfaceops.ai` may become product packaging for that direction, but current proof is limited to deterministic P4 artifacts plus the bounded local-loopback `surfaceops-kanban-live` adapter proof.
- `kanban.cards` should remain a standalone upstream collaboration-board substrate by default. SurfaceOps-specific governance, evidence refs, review queues, promotion semantics, and decision ownership should stay in SurfaceOps and flow to `kanban.cards` only through a SurfaceOps-owned adapter/projection layer. White-labeling `kanban.cards` as SurfaceOps remains a fallback only if standalone `kanban.cards` is intentionally deprioritized.
...
- Current proof status: P4 implements `surfaceops-decision-ledger.v0` as deterministic evidence, not a live console or durable review database. The separate `surfaceops-kanban-static` target implements an inert board projection over accepted P3/P4 evidence and a local substrate contract. The separate `surfaceops-kanban-live` target implements a bounded local-loopback live adapter proof over a real `kanban.cards` server, real API read/write operations, realtime observation, local persistence restart, permission-denial checks, and browser video evidence.
- Must not do: invent catalog policy, mutate upstream artifacts, execute work orders, persist live decisions as proof authority, infer decisions from kanban lane movement, expose secrets, or authorize protocol execution outside a target-specific proof.
- Future proof required before claim: production SurfaceOps needs schemas, fixtures, diagnostics, persistence rules, command/API boundaries, audit evidence, review workflows, production auth, hosted deployment, and proof gates. Production board sync remains unclaimed beyond the implemented local-loopback `surfaceops-kanban-live` target.
...
- Consumes: in the implemented `surfaceops-kanban-static` proof, only a manifest-declared local substrate contract is consumed as presentation shape. In the implemented `surfaceops-kanban-live` proof, a local `kanban.cards` server consumes real API writes for a bounded designer approval scenario.
- Emits: the static proof emits SurfaceOps-owned inert board projection and packet artifacts under `artifacts/surfaceops-kanban-static/`; the live proof emits deterministic adapter artifacts under `artifacts/surfaceops-kanban-live/` plus browser runtime evidence under `output/playwright/surfaceops-kanban-live/`.
- Current proof status: standalone product or repo direction, plus a local static substrate contract consumed by `surfaceops-kanban-static` and a bounded local-loopback API/browser integration consumed by `surfaceops-kanban-live`. The preferred packaging keeps `kanban.cards` upstream of SurfaceOps-specific governance, with SurfaceOps owning the adapter/projection layer and white-labeling as a fallback if standalone `kanban.cards` is intentionally deprioritized.
- Must not do: masquerade as proof authority, execute work orders, route review as SurfaceOps authority, persist SurfaceOps decisions as proof authority, create catalog policy, infer approvals from lane movement, or claim production proof status from local browser artifacts.
- Future proof required before claim: production `kanban.cards` integration still needs production product boundaries, production auth, hosted persistence, service-account permissions, schemas, fixtures, diagnostics, command or API contract, artifact/report/evidence paths, audit replay, broader conflict handling, and explicit non-goals beyond the implemented local-loopback `surfaceops-kanban-live` proof.
