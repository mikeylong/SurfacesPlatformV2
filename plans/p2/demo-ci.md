...
- Demo generation must verify the full evidence closure, including the immutable package snapshot lock, `sourceManifestRef`, `sourceFileRefs`, `schemaClosure`, `fixtureRefs`, `artifactRefs`, and the final self-hash.
...
That guard validates the schemas, immutable snapshot-lock contract and path, source-plan files, valid Spectrum fixtures, invalid/review/mutation fixture presence, diagnostic-code coverage, source and mapping ref normalization, evidence closure, package script wiring, and the `check:p2:untracked` guard. It is not proof evidence and must not be described as `check:p2:ci`.
...
`materialize:p2` treats `package-snapshot.lock.json` as an immutable input. It schema-validates the lock, compares the exact checked-in package paths and raw bytes with the lock, and fails on any mismatch before generated manifest metadata can be refreshed. The command never creates or rewrites the lock.

`check:p2:ci` must:

- materialize P0/P1/P2 schemas, fixtures, and declared source bundle metadata while comparing, but never regenerating, the immutable package snapshot lock;
- run P0 proof unchanged;
- run P1 proof unchanged;
- run P2 ingest proof;
- rebuild `demo/p2`;
- run the full test suite;
- fail if generated schemas, fixtures, artifacts, source inventory, source mapping, catalogs, evidence, demos, or the immutable lock ref drift;
...
## P2 Proof
The demo/CI layer passes only when P2 evidence passes, demo output rebuilds deterministically from evidence, tests cover the P2 ingestion proof contract and tamper cases, and drift checks confirm generated artifacts are current. Tamper coverage must change real package bytes while leaving the immutable lock unchanged, observe `INGEST_PACKAGE_SNAPSHOT_LOCK_MISMATCH`, and restore the original bytes. A fixture-only or manifest-only hash mutation is not causal package-byte coverage.
...
Later hardening work remains backlog, not current implementation. Track double-run determinism, additional ref-closure tampering variants, additional JSON Pointer negative cases, additional symlink cases, and additional evidence tampering variants as future cases extending existing coverage classes; each still needs schemas, fixtures, diagnostics, commands, artifacts, reports, and evidence before it can be claimed. The immutable package snapshot lock and causal package-byte tamper case are current P2 contract work, not backlog wording.

## Allowed Claims
With passing evidence, claims are scoped to deterministic local npm package ingestion for `@adobe/spectrum-design-data@0.7.0`, initial components `button` and `in-line-alert`, and the immutable-lock-verified declared source snapshot. P2 must not claim full Spectrum support, live ingestion, runtime adapter rendering, A2UI support, SurfaceOps operation, JudgmentKit evaluation, P3 orchestration, or Adobe endorsement.
