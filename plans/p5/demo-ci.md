...
## Status
This is the implemented P5 demo and CI contract for the `surfaces-protocol-static` and `surfaces-native-static` proofs.
...
The P5 native demo makes native proof artifacts inspectable, but it remains deterministic presentation output derived from passing P5 native evidence. It must not become proof authority, production native documentation, a native SDK, native bridge, live runtime, live API console, SurfaceOps product, JudgmentKit integration, or A2UI claim.
...
- Protocol evidence compatibility ref recorded in native evidence.

## Outputs
- `demo/p5/protocol/README.md`.
- `demo/p5/protocol/index.html`.
- `demo/p5/native/README.md`.
- `demo/p5/native/index.html`.
- Package scripts and CI gates for the `surfaces-protocol-static` and `surfaces-native-static` proofs.
...
- Native demo generation must require passing `artifacts/p5/native/evidence.json`.
- Native demo output may display native target selection, projection scope, inert packets, compatibility preflight refs, review-required and blocked outcomes, diagnostics, artifact refs, and evidence status.
- Native demo output must not include live forms, native bridge controls, API consoles, SDK snippets that imply runnable support, network fetches, connector calls, callbacks, secrets, live account identifiers, SurfaceOps decision submission, JudgmentKit execution controls, or A2UI conformance claims.
...
npm run materialize:p5:native
npm run proof:p5:native
npm run build:p5-native-demo
npm run check:p5:native
npm run check:p5:native:ci
npm run check:p5:native:untracked
```
...
The `check:p5:native:ci` gate first runs the cumulative protocol gate, then materializes native schemas and fixtures, runs the P5 native proof command, rebuilds `demo/p5/native`, runs tests, fails on generated drift, and fails if expected P5 native files are untracked.
...
- The declared P5 native proof artifact set must be the only allowed file set directly under `artifacts/p5/native`.
- `artifacts/p5/native/evidence.json` is stale when its self-hash is invalid, its `status` is not `pass`, its aggregate `promotionStatus` is `blocked`, an artifact, boundary ref, or compatibility-preflight ref points at a missing file, or any recomputed artifact hash differs from evidence.
- Demo generation must enumerate `artifacts/p5/native` and reject stale unexpected output before reading proof artifacts or writing demo files.
- CI must fail on untracked files under `artifacts/p5/native` because `git diff --exit-code` does not report untracked files.
...
- The native demo shows selected native target, inert native packets, compatibility preflight refs, review-required and blocked fixtures, diagnostics, provenance, hashes, and promotion status as proof output only.
- Re-running the native demo build from unchanged passing evidence produces no diff and no untracked files under both `artifacts/p5/native` and `demo/p5/native`.

## Non-Goals
- No hosted product site.
- No live interactive protocol console.
- No external assets.
- No screenshot baseline requirement in the first P5 protocol or native demos.
- No browser-driven accessibility audit.
- No production API documentation claim.
- No A2UI conformance claim.
- No claim that future P5 targets are implemented by these static protocol or native demos and CI gates.
- No production native documentation, native SDK, native bridge, live runtime, or claim that future P5 targets are implemented by this static native demo or CI gate.
