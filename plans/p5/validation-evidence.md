...
| `PROTOCOL_SOURCE_HASH_MISMATCH` | Protocol projection source hash does not match generated target selection or accepted evidence. | `projection` | `blocked` | `fixtures/p5/protocol/mutations/projection-hash-mismatch.protocol-projection.json` | `/catalogRef/hash` | `null` | `mutations/projection-hash-mismatch.protocol-projection.json` |
| `PROTOCOL_SOURCE_HASH_MISMATCH` | Protocol projection source hash does not match generated target selection or accepted evidence. | `projection` | `blocked` | `fixtures/p5/protocol/mutations/projection-target-selection-hash-mismatch.protocol-projection.json` | `/targetSelectionRef/hash` | `null` | `mutations/projection-target-selection-hash-mismatch.protocol-projection.json` |
| `PROTOCOL_TOKEN_RECORD_INVALID` | Protocol token records must stay closed over type, value, and sourceRef. | `projection` | `blocked` | `fixtures/p5/protocol/mutations/projection-token-extra-property.protocol-projection.json` | `/tokens/component-height-75/cssVariable` | `null` | `mutations/projection-token-extra-property.protocol-projection.json` |
| `PROTOCOL_TOKEN_RECORD_INVALID` | Protocol token records must stay closed over type, value, and sourceRef. | `projection` | `blocked` | `fixtures/p5/protocol/mutations/projection-token-missing-source-ref.protocol-projection.json` | `/tokens/component-height-75/sourceRef` | `null` | `mutations/projection-token-missing-source-ref.protocol-projection.json` |
...
| `PROTOCOL_TOKEN_RECORD_INVALID` | Protocol token records must stay closed over type, value, and sourceRef. | `protocol-boundary` | `blocked` | `fixtures/p5/protocol/mutations/envelope-token-extra-property.protocol-envelope.json` | `/tokens/height/cssProperty` | `null` | `mutations/envelope-token-extra-property.protocol-envelope.json` |
...
`schemas/surfaces-native-diagnostics.v0.schema.json`, `fixtures/p5/native/expectations.manifest.json`, `artifacts/p5/native/surfaces-native-report.json`, and `artifacts/p5/native/evidence.json` encode the native registry rows. Codes not listed there are invalid for the P5 native proof. The native registry includes `NATIVE_TOKEN_RECORD_INVALID` rows for projection token records with extra implementation fields, projection token records missing `sourceRef`, and packet token records with CSS runtime metadata.
