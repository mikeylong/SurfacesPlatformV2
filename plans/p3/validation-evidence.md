...
P3 generated artifact refs are acyclic. Forward refs to later same-run generated artifacts omit and forbid hashes; resolved backward refs to already materialized generated artifacts may include hashes. Final P3 evidence owns the complete hash closure for the orchestration plan, work orders, review queue, report, and evidence self-hash.
...
Command-level upstream preflight mutation fixtures under `fixtures/p3/mutations/*.agent-preflight.json` model missing, failing, hash-mismatched, and stale upstream evidence command invocations. They must fail before `agent-capability-registry.fixture.json` or any `*.agent-task.json` fixture is read, so their manifest rows point to the preflight mutation fixture itself rather than a task fixture.
...
| `AGENT_UNREGISTERED` | An orchestration plan or work order selects an agent id absent from the registry | Agent orchestration plan selects an unregistered agent. | `error` | `recruitment-validator` | `recruitment` | `recruitment-selection` | `fixtures/p3/mutations/unregistered-agent.agent-orchestration-plan.json` | `/tasks/0/selectedAgentIds/0` | `invalid` | `blocked` | `mutations/unregistered-agent.agent-orchestration-plan.json` |
| `AGENT_CAPABILITY_UNREGISTERED` | A task requires a capability absent from the registry | Agent task requires an unregistered capability. | `error` | `recruitment-validator` | `recruitment` | `recruitment-selection` | `fixtures/p3/invalid/unregistered-capability.agent-task.json` | `/requiredCapabilities` | `invalid` | `blocked` | `invalid/unregistered-capability.agent-task.json` |
...
| `AGENT_SCOPE_ESCALATION` | A task reads, writes, or emits outside declared artifact scope | Agent task exceeds declared scope. | `error` | `work-order-validator` | `work-order` | `work-order-validation` | `fixtures/p3/invalid/scope-escalation.agent-task.json` | `/allowedOutputs` | `invalid` | `blocked` | `invalid/scope-escalation.agent-task.json` |
| `AGENT_SCOPE_ESCALATION` | A generated work order reads, writes, or emits outside resolved artifact scope | Agent work order exceeds declared scope. | `error` | `work-order-validator` | `work-order` | `work-order-validation` | `fixtures/p3/mutations/work-order-scope-escalation.agent-work-order.json` | `/allowedOutputs` | `invalid` | `blocked` | `mutations/work-order-scope-escalation.agent-work-order.json` |
| `AGENT_OUTPUT_HIDDEN` | A task declares hidden, untracked, or non-evidence output | Agent task declares hidden output. | `error` | `work-order-validator` | `work-order` | `work-order-validation` | `fixtures/p3/invalid/hidden-output.agent-task.json` | `/allowedOutputs` | `invalid` | `blocked` | `invalid/hidden-output.agent-task.json` |
| `AGENT_REVIEW_POLICY_BLOCKED` | A task declares blocked review policy | Agent task review policy is blocked and cannot be promoted. | `error` | `review-router` | `review` | `review-routing` | `fixtures/p3/invalid/blocked-review-policy.agent-task.json` | `/reviewPolicy` | `invalid` | `blocked` | `invalid/blocked-review-policy.agent-task.json` |
| `AGENT_DEPENDENCY_CYCLE` | Orchestration plan contains a dependency cycle | Agent orchestration plan contains a dependency cycle. | `error` | `orchestration-validator` | `orchestration` | `orchestration-plan` | `fixtures/p3/invalid/cycle-dependency.agent-task.json` | `/dependencies` | `invalid` | `blocked` | `invalid/cycle-dependency.agent-task.json` |
| `AGENT_DEPENDENCY_MISSING` | Orchestration plan references a missing dependency | Agent orchestration plan references a missing dependency. | `error` | `orchestration-validator` | `orchestration` | `orchestration-plan` | `fixtures/p3/invalid/missing-dependency.agent-task.json` | `/dependencies` | `invalid` | `blocked` | `invalid/missing-dependency.agent-task.json` |
| `AGENT_TASK_DUPLICATE_ID` | Orchestration plan contains a duplicate task id | Agent orchestration plan contains a duplicate task id. | `error` | `orchestration-validator` | `orchestration` | `orchestration-plan` | `fixtures/p3/mutations/duplicate-task-id.agent-orchestration-plan.json` | `/tasks/1/taskId` | `invalid` | `blocked` | `mutations/duplicate-task-id.agent-orchestration-plan.json` |
| `AGENT_WORK_ORDER_DUPLICATE_ID` | Orchestration plan contains a duplicate work-order id | Agent orchestration plan contains a duplicate work-order id. | `error` | `orchestration-validator` | `orchestration` | `orchestration-plan` | `fixtures/p3/mutations/duplicate-work-order-id.agent-orchestration-plan.json` | `/workOrders/1/workOrderId` | `invalid` | `blocked` | `mutations/duplicate-work-order-id.agent-orchestration-plan.json` |
| `AGENT_HANDOFF_HIDDEN` | Orchestration plan contains a handoff not declared by task dependencies or evidence refs | Agent orchestration plan contains a hidden handoff. | `error` | `orchestration-validator` | `orchestration` | `orchestration-plan` | `fixtures/p3/mutations/hidden-handoff.agent-orchestration-plan.json` | `/tasks/1/dependencies/0` | `invalid` | `blocked` | `mutations/hidden-handoff.agent-orchestration-plan.json` |
| `AGENT_UPSTREAM_EVIDENCE_MISSING` | Command-level upstream P2 evidence input is missing | P3 upstream evidence input is missing. | `error` | `preflight-validator` | `preflight` | `upstream-preflight` | `fixtures/p3/mutations/missing-upstream-evidence.agent-preflight.json` | `/ingestionEvidenceRef` | `invalid` | `blocked` | `mutations/missing-upstream-evidence.agent-preflight.json` |
| `AGENT_UPSTREAM_EVIDENCE_FAILED` | Command-level upstream P2 evidence is not passing | P3 upstream evidence status is not pass. | `error` | `preflight-validator` | `preflight` | `upstream-preflight` | `fixtures/p3/mutations/failing-upstream-evidence.agent-preflight.json` | `/status` | `invalid` | `blocked` | `mutations/failing-upstream-evidence.agent-preflight.json` |
| `AGENT_UPSTREAM_EVIDENCE_HASH_MISMATCH` | Command-level upstream P2 evidence or catalog hash does not match the accepted boundary | P3 upstream evidence hash does not match the accepted boundary. | `error` | `preflight-validator` | `preflight` | `upstream-preflight` | `fixtures/p3/mutations/upstream-evidence-hash-mismatch.agent-preflight.json` | `/boundaryRefs/0/hash` | `invalid` | `blocked` | `mutations/upstream-evidence-hash-mismatch.agent-preflight.json` |
| `AGENT_UPSTREAM_EVIDENCE_STALE` | Command-level upstream P2 evidence or catalog ref is stale or not the exact declared input | P3 upstream evidence boundary is stale. | `error` | `preflight-validator` | `preflight` | `upstream-preflight` | `fixtures/p3/mutations/stale-upstream-evidence.agent-preflight.json` | `/boundaryRefs` | `invalid` | `blocked` | `mutations/stale-upstream-evidence.agent-preflight.json` |
...
3. If any review-required or blocked fixture emits executable work or is promoted as allowed, `status` is `fail` and `promotionStatus` is `blocked`.
