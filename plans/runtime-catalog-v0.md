...
`compatibility` may mention A2UI only as reference-only unless a separate A2UI-specific P5 proof exists. It must not claim adapter compatibility.
...
- No broad A2UI compatibility layer without a separate A2UI-specific P5 proof.
...
- P1 resolves the first adapter proof target as `web-static` over `runtime-projection.v0`; additional adapter target selection remains deferred, and A2UI remains deferred to a future A2UI-specific P5 target.
