...
async function verifyEvidenceClosure({ cwd, evidence, evidencePath }) {
  ...
  if (evidence.contractId === "surfaces-source-family-component-identity-mapping-proof") {
    const { sourceFamilyComponentIdentityMappingInternals } = await import("./source-family-component-identity-mapping-proof.js");
    if (await sourceFamilyComponentIdentityMappingInternals.firstEvidenceIntegrityFailureCode(cwd, evidence) !== null) {
      throw diagnosticError("CAPABILITY_EVIDENCE_HASH_MISMATCH", evidencePath);
    }
  }
