...
export async function runInterfacectl(argv, io) {
  if (argv[0] === "surfaces" && argv[1] === "capabilities") {
    const { runCapabilityIndexInterfacectl } = await import("./capability-index-proof.js");
    return runCapabilityIndexInterfacectl(argv.slice(2), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "surfaceops-designer-review-ui" && argv[2] === "proof") {
    const { runSurfaceopsDesignerReviewUiInterfacectl } = await import("./surfaceops-designer-review-ui-proof.js");
    return runSurfaceopsDesignerReviewUiInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "surfaceops-kanban-static" && argv[2] === "proof") {
    const { runSurfaceopsKanbanStaticInterfacectl } = await import("./surfaceops-kanban-static-proof.js");
    return runSurfaceopsKanbanStaticInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "surfaceops-kanban-live" && argv[2] === "proof") {
    const { runSurfaceopsKanbanLiveInterfacectl } = await import("./surfaceops-kanban-live-proof.js");
    return runSurfaceopsKanbanLiveInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "designer-workflow-trace" && argv[2] === "proof") {
    const { runDesignerWorkflowTraceInterfacectl } = await import("./designer-workflow-trace-proof.js");
    return runDesignerWorkflowTraceInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "source-conformance" && argv[2] === "proof") {
    const { runSourceConformanceInterfacectl } = await import("./source-conformance-proof.js");
    return runSourceConformanceInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "source-family-packaging" && argv[2] === "proof") {
    const { runSourceFamilyPackagingInterfacectl } = await import("./source-family-packaging-proof.js");
    return runSourceFamilyPackagingInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "source-family-layout-mapping" && argv[2] === "proof") {
    const { runSourceFamilyLayoutMappingInterfacectl } = await import("./source-family-layout-mapping-proof.js");
    return runSourceFamilyLayoutMappingInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "source-family-namespace-mapping" && argv[2] === "proof") {
    const { runSourceFamilyNamespaceMappingInterfacectl } = await import("./source-family-namespace-mapping-proof.js");
    return runSourceFamilyNamespaceMappingInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "source-family-component-identity-mapping" && argv[2] === "proof") {
    const { runSourceFamilyComponentIdentityMappingInterfacectl } = await import("./source-family-component-identity-mapping-proof.js");
    return runSourceFamilyComponentIdentityMappingInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "source-accessibility-policy" && argv[2] === "proof") {
    const { runSourceAccessibilityPolicyInterfacectl } = await import("./source-accessibility-policy-proof.js");
    return runSourceAccessibilityPolicyInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "spectrum-checkbox-catalog" && argv[2] === "proof") {
    const { runSpectrumCheckboxCatalogInterfacectl } = await import("./spectrum-checkbox-catalog-proof.js");
    return runSpectrumCheckboxCatalogInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "design-system-compiler" && argv[2] === "proof") {
    const { runDesignSystemCompilerInterfacectl } = await import("./design-system-compiler-proof.js");
    return runDesignSystemCompilerInterfacectl(argv.slice(3), io);
  }

  if (argv[0] === "surfaces" && argv[1] === "native" && argv[2] === "proof") {
    const { runP5NativeInterfacectl } = await import("./p5-native-proof.js");
    return runP5NativeInterfacectl(argv.slice(3), io);
  }
  if (argv[0] === "surfaces" && argv[1] === "protocol" && argv[2] === "proof") {
    const { runP5ProtocolInterfacectl } = await import("./p5-protocol-proof.js");
    return runP5ProtocolInterfacectl(argv.slice(3), io);
  }
  ...
  if (argv[0] !== "surfaces" || argv[1] !== "proof") {
    io.stderr.write("usage: interfacectl surfaces source-family-layout-mapping proof --source sources/source-family-layout-mapping/team-owned-physical-bundle --mapping sources/source-family-layout-mapping/layout-mapping.json --layout-package fixtures/source-family-layout-mapping/layout-package.fixture.json --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --source-family-packaging-evidence artifacts/source-family-packaging/evidence.json --fixture fixtures/source-family-layout-mapping --out artifacts/source-family-layout-mapping\n");
    io.stderr.write("usage: interfacectl surfaces source-family-namespace-mapping proof --source sources/source-family-namespace-mapping/team-owned-namespaced-bundle --mapping sources/source-family-namespace-mapping/namespace-mapping.json --namespace-package fixtures/source-family-namespace-mapping/namespace-package.fixture.json --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --source-family-layout-mapping-evidence artifacts/source-family-layout-mapping/evidence.json --fixture fixtures/source-family-namespace-mapping --out artifacts/source-family-namespace-mapping\n");
    io.stderr.write("usage: interfacectl surfaces source-family-component-identity-mapping proof --source sources/source-family-component-identity-mapping/team-owned-identity-bundle --authority-manifest sources/source-family-component-identity-mapping/authority/component-identity-authority-manifest.json --authority-declaration sources/source-family-component-identity-mapping/authority/component-identity-declaration.json --mapping sources/source-family-component-identity-mapping/component-identity-mapping.json --identity-package fixtures/source-family-component-identity-mapping/component-identity-package.fixture.json --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --source-family-namespace-mapping-evidence artifacts/source-family-namespace-mapping/evidence.json --fixture fixtures/source-family-component-identity-mapping --out artifacts/source-family-component-identity-mapping\n");
    io.stderr.write("usage: interfacectl surfaces source-accessibility-policy proof --source sources/source-accessibility-policy --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --source-conformance-evidence artifacts/source-conformance/evidence.json --source-conformance-catalog artifacts/source-conformance/governed-catalog.json --source-family-packaging-evidence artifacts/source-family-packaging/evidence.json --fixture fixtures/source-accessibility-policy --out artifacts/source-accessibility-policy\n");
    io.stderr.write("usage: interfacectl surfaces spectrum-checkbox-catalog proof --source sources/spectrum-checkbox-catalog --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --fixture fixtures/spectrum-checkbox-catalog --out artifacts/spectrum-checkbox-catalog\n");
    io.stderr.write("usage: interfacectl surfaces design-system-compiler proof --manifest fixtures/design-system-compiler/targets.manifest.json --out artifacts/design-system-compiler\n");
    io.stderr.write("usage: interfacectl surfaces capabilities proof --fixture fixtures/capability-index --out artifacts/capability-index\nusage: interfacectl surfaces capabilities verify --index artifacts/capability-index/capability-index.json --evidence artifacts/capability-index/evidence.json\nusage: interfacectl surfaces proof --fixture fixtures/p0 --out artifacts/p0\nusage: interfacectl surfaces adapter proof --catalog artifacts/p0/governed-catalog.json --fixture fixtures/p1 --out artifacts/p1\nusage: interfacectl surfaces ingest proof --source sources/p2/design-system-source --fixture fixtures/p2 --out artifacts/p2\nusage: interfacectl surfaces agents proof --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --fixture fixtures/p3 --out artifacts/p3\nusage: interfacectl surfaces review proof --orchestration-evidence artifacts/p3/evidence.json --review-queue artifacts/p3/review-queue.json --fixture fixtures/p4 --out artifacts/p4\nusage: interfacectl surfaces protocol proof --ingestion-evidence artifacts/p2/evidence.json --review-evidence artifacts/p4/evidence.json --decision-ledger artifacts/p4/surfaceops-decision-ledger.json --review-report artifacts/p4/review-judgment-report.json --catalog artifacts/p2/governed-catalog.json --fixture fixtures/p5/protocol --out artifacts/p5/protocol\nusage: interfacectl surfaces native proof --ingestion-evidence artifacts/p2/evidence.json --review-evidence artifacts/p4/evidence.json --decision-ledger artifacts/p4/surfaceops-decision-ledger.json --review-report artifacts/p4/review-judgment-report.json --catalog artifacts/p2/governed-catalog.json --protocol-evidence artifacts/p5/protocol/evidence.json --fixture fixtures/p5/native --out artifacts/p5/native\nusage: interfacectl surfaces source-conformance proof --source sources/source-conformance/declared-source-bundle --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --fixture fixtures/source-conformance --out artifacts/source-conformance\nusage: interfacectl surfaces source-family-packaging proof --package fixtures/source-family-packaging/package.fixture.json --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --source-conformance-evidence artifacts/source-conformance/evidence.json --source-conformance-catalog artifacts/source-conformance/governed-catalog.json --fixture fixtures/source-family-packaging --out artifacts/source-family-packaging\nusage: interfacectl surfaces designer-workflow-trace proof --ingestion-evidence artifacts/p2/evidence.json --catalog artifacts/p2/governed-catalog.json --ingestion-report artifacts/p2/ingestion-report.json --source-conformance-evidence artifacts/source-conformance/evidence.json --source-authority-map artifacts/source-conformance/source-authority-map.json --source-conformance-report artifacts/source-conformance/source-conformance-report.json --source-review-queue artifacts/source-conformance/source-review-queue.json --orchestration-evidence artifacts/p3/evidence.json --review-queue artifacts/p3/review-queue.json --review-evidence artifacts/p4/evidence.json --decision-ledger artifacts/p4/surfaceops-decision-ledger.json --review-report artifacts/p4/review-judgment-report.json --evaluation-report artifacts/p4/judgmentkit-evaluation-report.json --protocol-evidence artifacts/p5/protocol/evidence.json --native-evidence artifacts/p5/native/evidence.json --fixture fixtures/designer-workflow-trace --out artifacts/designer-workflow-trace\nusage: interfacectl surfaces surfaceops-kanban-static proof --orchestration-evidence artifacts/p3/evidence.json --review-queue artifacts/p3/review-queue.json --orchestration-report artifacts/p3/agent-orchestration-report.json --review-evidence artifacts/p4/evidence.json --decision-ledger artifacts/p4/surfaceops-decision-ledger.json --review-report artifacts/p4/review-judgment-report.json --evaluation-report artifacts/p4/judgmentkit-evaluation-report.json --kanban-substrate-manifest sources/surfaceops-kanban-static/kanban-substrate-manifest.json --fixture fixtures/surfaceops-kanban-static --out artifacts/surfaceops-kanban-static\nusage: interfacectl surfaces surfaceops-kanban-live proof --orchestration-evidence artifacts/p3/evidence.json --review-queue artifacts/p3/review-queue.json --orchestration-report artifacts/p3/agent-orchestration-report.json --review-evidence artifacts/p4/evidence.json --decision-ledger artifacts/p4/surfaceops-decision-ledger.json --review-report artifacts/p4/review-judgment-report.json --evaluation-report artifacts/p4/judgmentkit-evaluation-report.json --kanban-api-manifest sources/surfaceops-kanban-live/kanban-cards-api-manifest.json --fixture fixtures/surfaceops-kanban-live --out artifacts/surfaceops-kanban-live\nusage: interfacectl surfaces surfaceops-designer-review-ui proof --designer-evidence artifacts/designer-workflow-trace/evidence.json --designer-report artifacts/designer-workflow-trace/designer-workflow-trace-report.json --review-evidence artifacts/p4/evidence.json --decision-ledger artifacts/p4/surfaceops-decision-ledger.json --kanban-live-evidence artifacts/surfaceops-kanban-live/evidence.json --kanban-live-operation-plan artifacts/surfaceops-kanban-live/surfaceops-kanban-live-operation-plan.json --fixture fixtures/surfaceops-designer-review-ui --out artifacts/surfaceops-designer-review-ui\n");
    return 2;
  }
