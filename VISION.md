...
The current roadmap evidence is P0-P5, with P5 implemented only for the `surfaces-protocol-static` and `surfaces-native-static` proof-only slices. This repo also implements target-specific declared-source conformance, designer-workflow-trace, `surfaceops-kanban-static`, and `surfaceops-kanban-live` proofs that consume accepted evidence; none is a new numbered phase. `surfaceops-kanban-live` is a bounded local-loopback live `kanban.cards` integration proof, not a production SurfaceOps or production `kanban.cards` claim. A row counts as implemented only when the named evidence file records `status: "pass"`. `promotionStatus` records governance outcome, not whether the proof command failed.
