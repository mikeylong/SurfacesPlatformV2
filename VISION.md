...
The current roadmap evidence is P0-P5, with P5 implemented only for the `surfaces-protocol-static` and `surfaces-native-static` proof-only slices. This repo also implements target-specific declared-source conformance, structured accessibility policy reconciliation, fixed source-family layout mapping, designer-workflow-trace, `surfaceops-kanban-static`, `surfaceops-kanban-live`, `surfaceops-designer-review-ui`, and capability-index proofs; none is a new numbered phase. The capability index covers exactly the 14 proof targets that existed before it and does not index itself. Its separate planned capability groups provide roadmap visibility only. `surfaceops-kanban-live` and `surfaceops-designer-review-ui` are bounded local-loopback live proofs, not production SurfaceOps or production `kanban.cards` claims. A row counts as implemented only when the named evidence file records `status: "pass"`. `promotionStatus` records governance outcome, not whether the proof command failed.
...
| Target | Capability index and read-only verifier | Discovery and integrity verification across the 14 implemented proof targets, with planned groups kept separate from implementation claims | `artifacts/capability-index/evidence.json` | `pass` | `allowed` | `npm run check:capability-index:ci` | none; report/evidence only |
...
over exactly the 14 pre-existing implemented proof targets. Its read-only
...
- Broader declared-source conformance remains open beyond the two checked instances of the fixed local source-family package. The package proof establishes compiler reuse for Button and InLineAlert without family-specific JavaScript, but arbitrary layouts or source-ref namespaces, broader component coverage, live connectors, self-serve connection, and production-facing conformance still require separate proof shapes and passing evidence.
